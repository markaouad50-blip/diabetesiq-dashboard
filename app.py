...
def fig_global_trend(trend, h=340):
    ...
    fig.update_xaxes(gridcolor="rgba(0,0,0,0)")    # no vertical grid on time series
    return fig
...
def fig_region_bar(reg, h=340):
    ...
    fig.update_xaxes(range=[0, df["Prevalence"].max() * 1.15])
...
def fig_age_gender(demo, h=340):
    ...
    fig.update_xaxes(tickfont=dict(color=BLUE_DIM, size=10), gridcolor="rgba(0,0,0,0)")
    return fig
...
def fig_feature_importance(imp_df, h=340):
    ...
    fig.update_xaxes(range=[0, df["Importance"].max() * 1.18])
    fig.update_yaxes(tickfont=dict(color=BLUE_MID, size=11), gridcolor="rgba(0,0,0,0)")
    return fig
...
def page_global_map(map_df):
    ...
    fig_top.update_xaxes(range=[0, top20["Prevalence"].max() * 1.15])
...
def page_trends(map_df, trend, regional):
    ...
    with tab1:
        ...
        # Headroom so the outside "%{text}M" labels aren't clipped / leaving gaps.
        fig_abs.update_yaxes(range=[0, trend_cat["Adults_M"].max() * 1.16])
        fig_abs.update_xaxes(gridcolor="rgba(0,0,0,0)")
        ...
        if map_df.empty:
            ...
            if not selected:
                ...
                fig_cmp.update_layout(
                    ...
                    **_layout(400, b=44))
                # Headroom for outside labels; trim wasted vertical band below axis.
                fig_cmp.update_yaxes(range=[0, sub["Prevalence"].max() * 1.18])
                fig_cmp.update_xaxes(tickangle=-35, gridcolor="rgba(0,0,0,0)",
    ...
    with tab3:
        ...
        with c2:
            ...
            fig_u.update_xaxes(range=[0, un["Undiagnosed"].max() * 1.15])
