...
def _layout(h=360, t=44, b=36, l=10, r=10, grid="none"):
    ...
    Gridlines are removed from every chart — they are non-data ink. The `grid`
    argument is retained for call-site compatibility but no longer draws anything.
    """
    c = _c()

    def axis(show_grid):
        return dict(
            showgrid=False,                       # no gridlines anywhere
...
def page_predictive(pima):
    ...
    st.markdown(
        f"<p style='display:flex;align-items:flex-start;gap:7px;color:{RISK_MID};"
        f"font-size:.8rem;margin-bottom:1rem;line-height:1.5;'>{icon('risk', 15, 2)}"
        "<span>For educational purposes only — not a clinical diagnostic tool. "
        "The model is trained on a <strong>female-only</strong> clinical cohort "
        "(Pima Indians dataset), so estimates are most applicable to women.</span></p>",
        unsafe_allow_html=True)
